...
lang Fragments = PreToken + PreLitToken + MExprCmp + MExprPrettyPrint + CarriedBasic + MExprEq + LRParser + MLangAst + MkOpLanguages + LRParser
end
...
let runParserGenerator : {synFile : String, outFile : String} -> () = lam args.
  ...
  let desugarAndResolveTokenDecl
    ...
    = lam x.
      ...
      result.withAnnotations
        ...
        (result.apply
          (result.map4
            (lam constructor. lam fragment. lam ty. lam base. lam wrap.
  ...
  let desugaredTokenToTokenInfo
    ...
    = lam surround. lam name. lam record.
      ...
      match name with Some name then
        ...
        switch (record.constructor, record.base)
        case (Some (_, constructor), None ()) then
          let ty = match record.ty with Some (_, ty) then ty else tyWithInfo name.i (ntycon_ name.v) in
          let tinfo =
            { ty = ty
            , repr = PreRepr {constructorName = constructor.v}
            , bareTy = ty
          ...
          let f: {v: Name, i: Info} -> Res TokenInfo = lam name.
            match mapLookup name.v (deref typeMap) with Some tinfo then
              match tinfo with Right tinfo then
                result.map
                  (lam tinfo: TokenInfo.
                    { tinfo with getValue = wrap tinfo.getValue
                    , ty = match record.ty with Some (_, ty) then ty else tinfo.ty
                    })
                  tinfo
        ...
        case _ then
          (frag, Some (result.err (simpleMsg surround "A named token declaration must have 'constructor', or 'base'.\n")))
        end
    ...
    = lam decl.
      ...
      case TypeDecl x then
        ...
        let f = lam name: {v: Name, i: Info}. lam x: TypeDeclDesugaredRecord.
          { ty = ntycon_ name.v
          , opSynName = nameSym (concat (nameGetStr name.v) "Op")
          , ensureSuffix = true
  ...
  type PartialProduction =
    { record : RecordInfo
    -- `param` is the parameter to be added to the semantic action for
    -- the Production, `info` is an expression that evaluates to the
    -- `Info` field of the parsed symbol, and `sym` is the actual
    -- symbol to be added to the `Production`
  ...
  let mkRecordOfSeqsSymbol
    ...
    = lam nt. lam record.
      let paramName = nameSym "s" in
      let mkSymbol = lam ty.
        { param = (paramName, ty)
        , info = recordproj_ infoFieldLabel (nvar_ paramName)
        , sym = NonTerminal nt
        } in
      { record = record
      , symbols = [result.map mkSymbol (mkRecordOfSeqsTy record)]
      , terms = [result.ok (recordproj_ termsFieldLabel (nvar_ paramName))]
      , fields = mapMapWithKey (lam k. lam. [result.ok (recordproj_ k (nvar_ paramName))]) record
      }
  in
        ...
        = lam symbols. lam terms. lam fields.
          match foldl
            (lam acc. lam x : PartialSymbol.
              match acc with (param, info, sym) in
              (snoc param x.param, snoc info x.info, snoc sym x.sym))
            ([], [], [])
            symbols
          with (params, infos, syms) in
          let action: Expr =
            ...
            let res = withType
              (tyarrow_ stateTy
                (foldr (lam sym. lam acc. tyarrow_ sym.param.1 acc) resTy symbols))
              res in
            res
          in
          { nt = nt
  ...
  let processTerminal
    ...
    = lam term. switch term
      case NtTerm conf then
        ...
        let pairTy = tytuple_ [tycon_ "Info", ntycon_ conf.name] in
        let sym =
          { param = (pairName, pairTy)
          , info = tupleproj_ 0 (nvar_ pairName)
          , sym = NonTerminal conf.name
          } in
      ...
      case TokenTerm config then
        ...
        let ty = result.map
          (lam config: TokenInfo. tyrecord_ [("v", config.ty), ("i", tycon_ "Info")])
          config in
        let bareTy = result.map
          (lam config: TokenInfo. tyrecord_ [("v", config.bareTy), ("i", tycon_ "Info")])
          config in
        let sym = result.map2
          (lam config: TokenInfo. lam bareTy.
            { sym = Terminal config.repr
            , param = (valName, bareTy)
        ...
        let val = result.map
          ...
          config in
        (sym, [info], val, result.map untargetableType ty)
  ...
  let computeConstructorName
    : Name -> {v: Name, i: Info} -> TypeInfo -> Name
    = lam nt. lam constr. lam config.
      if config.ensureSuffix then
        if isSuffix eqc (nameGetStr nt) (nameGetStr constr.v)
        then constr.v
        else nameSym (concat (nameGetStr constr.v) (nameGetStr nt))
      else constr.v
  in

  -- NOTE(vipa, 2022-04-01): Figure out the operatorness of a production
  let findOperator
    : ProductionDeclRecord -> TypeInfo -> Name -> [SRegex] -> Res Operator
    = lam x. lam tinfo. lam name. lam reg.
      let temp =
  ...
  let constructors : Res [ConstructorInfo] =
    let check = lam decl.
      match decl with ProductionDecl x then
        let tinfo = switch mapFindExn x.nt.v typeMap
          case Left tinfo then tinfo
          case Right _ then result.err (x.nt.i, join ["The type of a production must be a type, not a token.\n", simpleHighlight x.nt.i, "\n"])
          end in
        let name = result.map (computeConstructorName x.nt.v x.name) tinfo in
        ...
        let operator = result.bind3 tinfo name reg (findOperator x) in
        ...
        match tinfo.grouping with Some (lpar, rpar) then
          ...
          let f : PartialSymbol -> PartialSymbol -> PartialSymbol -> Expr -> GenOperator
            = lam lPartSym. lam ntSym. lam rPartSym. lam ntVal.
              ...
              let action =
                let fn = ulam_ ""
                  (nlam_ lPartSym.param.0 lPartSym.param.1
                    (nlam_ ntSym.param.0 ntSym.param.1
                      (nlam_ rPartSym.param.0 rPartSym.param.1
                        (nconapp_ conName
                          (urecord_
  ...
  let composedParseFragmentName = nameSym (join ["Parse", langName, "Pre"]) in
  ...
  let parseFunctions : Res [Decl] =
    let lits : Res (Map String Name) =
      let collectLit = lam acc. lam term.
        match term with Terminal (PreLitRepr x)
        then mapUpdate x.lit (optionOrElse (lam. Some (nameSym x.lit))) acc
        else acc in
      result.map
        (foldl (lam acc. lam prod. foldl collectLit acc prod.terms) (mapEmpty cmpString))
        productions in
    let nextTokenName = nameSym "nextTokOrLit" in
    let fragmentIncludingLits : Res Decl = result.map
      (lam lits. DeclLang
        { ident = composedFullParseFragmentName
        , includes = [composedParseFragmentName]
        , info = NoInfo ()
        , decls =
          let constrs = mapValues lits in
          [ DeclSyn
            { ident = nameNoSym "Token"
            , params = []
            , defs = map (lam c. {ident = c, tyIdent = tycon_ "Info"}) constrs
            , info = NoInfo ()
            }
          , DeclSem
            { ident = nameNoSym "tokInfo"
            , tyAnnot = tyunknown_
            , tyBody = tyunknown_
            , args = []
            , cases = map (lam c. {pat = npcon_ c (pvar_ "i"), thn =var_ "i"}) constrs
            , info = NoInfo ()
            }
          , DeclSem
            { ident = nameNoSym "tokToStr"
            , tyAnnot = tyunknown_
            , tyBody = tyunknown_
            , args = []
            , cases = map (lam c. {pat = npcon_ c pvarw_, thn = str_ (join ["'", nameGetStr c, "'"])}) constrs
            , info = NoInfo ()
            }
          , DeclSem
            { ident = nextTokenName
            ...
            , cases =
              [ { pat = pvar_ "stream"
                , thn = bindall_
                  [ ulet_ "stream" (appf2_ (var_ "eatWSAC") (recordproj_ "pos" (var_ "stream")) (recordproj_ "str" (var_ "stream")))
                  , ulet_ "str" (recordproj_ "str" (var_ "stream"))
                  , let x = mapBindings lits in
                    let x = sort (lam a. lam b. subi (length a.0) (length b.0)) x in  -- NOTE(vipa, 2023-08-31): longest last
                    let addCase = lam acc. lam pair.
                      let advanceFunc = lam name. lam p. lam i. appf2_ (var_ name) p (int_ i) in
      ...
      lits in
    let parseNormalName = nameNoSym (concat "parse" langName) in
    let parseNormal : Res Decl =
      let fileName = nameSym "filename" in
      let contentName = nameSym "content" in
      let tokenInfo : Res (Map TokenRepr {conIdent: Name, conArg: Type}) =
        let res = mapValues typeMap in
        let f = lam e.
      ...
      in
      let tokenInfo = result.map2
        (lam tokenInfo. lam lits.
          mapFoldWithKey
            (lam acc. lam lit. lam name.
              mapInsert (PreLitRepr {lit = lit}) {conIdent = name, conArg = tycon_ "Info"} acc)
            tokenInfo
            lits)
        tokenInfo
        lits in
      ...
      let wrap = lam genOpResult. lam func.
        let initLexerState = urecord_
          ...
          , ("str", nvar_ contentName)
          ] in
        DeclLet
          { ident = parseNormalName
          , tyAnnot = tyunknown_
          , tyBody = tyunknown_
          , body = TmUse
            { ident = composedFullParseFragmentName
            , ty = tyunknown_
            , info = NoInfo ()
            , inexpr = genOpResult.wrapProductions
              (nulam_ fileName
                (nulam_ contentName
                  (bind_ (ulet_ "stream" initLexerState) func)))
            }
          , info = NoInfo ()
          }
      in
      result.map2 wrap genOpResult func
    in
    use BootParser in
    let parse = parseMExprString {_defaultBootParserParseMExprStringArg () with allowFree = true} in
    let parseExn : Decl = DeclLet
      { ident = nameNoSym (join ["parse", langName, "Exn"])
      , tyAnnot = tyunknown_
      , tyBody = tyunknown_
      , body = parse (strJoin "\n"
        ...
        , "switch res"
        , "case Left es then"
        , "  for_ es (lam x. printLn (infoErrorString x.0 x.1));"
        , "  exit 1"
        , "case Right res then res"
        , "end"
        ])
      , info = NoInfo ()
      } in
    result.map2 (lam fil. lam pn. [fil, pn, parseExn]) fragmentIncludingLits parseNormal
  in
  -- let parseFunctions : Res [Decl] =
  --   let f = lam start.
  --     use LetDeclAst in
  ...
  let generated: Res [Decl] = result.bind5 constructors badConstructors requestedFieldAccessors genOpResult parseFunctions
    (lam constructors : [ConstructorInfo]. lam badConstructors. lam requestedFieldAccessors. lam genOpResult : GenOpResult. lam parseFunctions.
      let genInput =
      ...
      result.ok (join
        [ [ DeclInclude {path = "seq.mc", info = NoInfo ()}
          , DeclInclude {path = "parser/lexer.mc", info = NoInfo ()}
  ...
  match result.consume (result.withAnnotations allResolved generated) with (warnings, res) in
