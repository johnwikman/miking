...
extern "C" {
	value gpuhost_fun74_factidx(value packedInts, value packedFloats, value arg0);
}

__device__ int gpudevice_fun74_factidx(int arg72_i, int arg73_ignored_arg);
__device__ int gpudevice_fun59_factorial(int arg60_n);
...
__device__ int gpudevice_fun74_factidx(int arg72_i, int arg73_ignored_arg)
{
	return gpudevice_fun59_factorial(arg72_i);
}

__device__ int gpudevice_fun59_factorial(int arg60_n)
{
	return (gpu_eqi(arg60_n, 0)) ? (1) : (gpu_muli(arg60_n, gpudevice_fun59_factorial(gpu_subi(arg60_n, 1))));
}
...
__global__ void gpuglobal_fun74_factidx(value *cuda_arg0, value *outarr, int n, int elemPerThread)
{
	int i;
	...
	for (i = start; i < end; ++i) {
		...
		outarr[i] = Val_int(gpudevice_fun74_factidx(i, v));
	}
}
...
value gpuhost_fun74_factidx(value packedInts, value packedFloats, value arg0)
{
	...
	gpuglobal_fun74_factidx<<<blockCount,threadsPerBlock>>>(cuda_arg0, cuda_outarr, n, elemPerThread);
