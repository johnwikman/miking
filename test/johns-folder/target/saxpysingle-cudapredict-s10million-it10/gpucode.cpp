...
extern "C" {
	value gpuhost_fun71_saxpy(value packedInts, value packedFloats, value arg0);
}

__device__ int gpudevice_fun71_saxpy(int arg68_a, int arg69_y, int arg70_xelem);
__device__ inline int gpu_muli(int x, int y);
__device__ inline int gpu_addi(int x, int y);

__device__ int gpudevice_fun71_saxpy(int arg68_a, int arg69_y, int arg70_xelem)
{
	return gpu_addi(gpu_muli(arg68_a, arg70_xelem), arg69_y);
}
...
__global__ void gpuglobal_fun71_saxpy(int cuda_arg0, int cuda_arg1, value *cuda_arg2, value *outarr, int n, int elemPerThread)
{
	int i;
	...
	for (i = start; i < end; ++i) {
		...
		outarr[i] = Val_int(gpudevice_fun71_saxpy(cuda_arg0, cuda_arg1, v));
	}
}
...
value gpuhost_fun71_saxpy(value packedInts, value packedFloats, value arg0)
{
	...
	gpuglobal_fun71_saxpy<<<blockCount,threadsPerBlock>>>(Int_val(Field(packedInts, 1)), Int_val(Field(packedInts, 2)), cuda_arg0, cuda_outarr, n, elemPerThread);
