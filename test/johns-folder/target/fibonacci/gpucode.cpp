...
extern "C" {
	value gpuhost_fun67_fib(value packedInts, value packedFloats);
}

__device__ inline int gpu_addi(int x, int y);
__device__ inline bool gpu_eqi(int x, int y);
__device__ int gpudevice_fun62_helper(int arg66_n, int arg63_i, int arg64_prev, int arg65_current);
__device__ int gpudevice_fun67_fib(int arg61_n);
...
__device__ int gpudevice_fun62_helper(int arg66_n, int arg63_i, int arg64_prev, int arg65_current)
{
	return (gpu_eqi(arg63_i, arg66_n)) ? (arg65_current) : (gpudevice_fun62_helper(arg66_n, gpu_addi(arg63_i, 1), arg65_current, gpu_addi(arg64_prev, arg65_current)));
}

__device__ int gpudevice_fun67_fib(int arg61_n)
{
	return gpudevice_fun62_helper(arg61_n, 0, 1, 0);
}

__global__ void gpuglobal_fun67_fib(value *outarr, int n, int elemPerThread)
{
	int i;
	...
	for (i = start; i < end; ++i) {
		outarr[i] = Val_int(gpudevice_fun67_fib(i));
	}
}
...
value gpuhost_fun67_fib(value packedInts, value packedFloats)
{
	...
	gpuglobal_fun67_fib<<<blockCount,threadsPerBlock>>>(cuda_outarr, n, elemPerThread);
