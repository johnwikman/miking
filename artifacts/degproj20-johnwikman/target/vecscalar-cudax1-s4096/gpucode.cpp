...
extern "C" {
	value gpuhost_fun80_vecscalef(value packedInts, value packedFloats, value arg0);
}

__device__ inline double gpu_mulf(double x, double y);
__device__ double gpudevice_fun80_vecscalef(double arg78_a, double arg79_xelem);

__device__ inline double gpu_mulf(double x, double y) {return x * y;}

__device__ double gpudevice_fun80_vecscalef(double arg78_a, double arg79_xelem)
{
	return gpu_mulf(arg78_a, arg79_xelem);
}

__global__ void gpuglobal_fun80_vecscalef(double cuda_arg0, value *cuda_arg1, value *outarr, int n, int elemPerThread)
{
	int i;
	...
	for (i = start; i < end; ++i) {
		...
		((double *) outarr)[i] = gpudevice_fun80_vecscalef(cuda_arg0, v);
	}
}
...
value gpuhost_fun80_vecscalef(value packedInts, value packedFloats, value arg0)
{
	...
	gpuglobal_fun80_vecscalef<<<blockCount,threadsPerBlock>>>(Double_field(packedFloats, 0), cuda_arg0, cuda_outarr, n, elemPerThread);
